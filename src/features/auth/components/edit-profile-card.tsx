...
import { FaEdit } from "react-icons/fa"; // Importing pencil icon
import { getCurrentUser, updateCurrentUser } from "@/services/userService"; // Importing services
import { useRouter } from "next/navigation"; // To handle navigation and reload
...
export const UpdateProfileForm = () => {
  ...
  const [imageFile, setImageFile] = useState<File | null>(null);
  const [message, setMessage] = useState<string | null>(null); // Message state for notifications
  const [isError, setIsError] = useState(false); // Error state
  const router = useRouter(); // Next.js router for navigation and reloading
  ...
  const handleEditClick = async () => {
    ...
    try {
      await updateCurrentUser({ nickName: userData.name }, imageFile); // Update with allowed fields
      setIsError(false);
